...
## Scenario Monte-Carlo data inventory

Goal: inventory for a linearized scenario Monte Carlo — synthetic (load, wind, solar) draws pushed through stored shift factors to estimate P(bind) / lambda distributions per constraint. All read-only, small queries (DESCRIBE/SHOW/LIMIT 3/single aggregates). Snowflake via `awconnect.snowflake.performQuery` (warehouse `AI_READ_ONLY`), SQL Server via `awconnect.db.getDfFromAwDb(database="AW")`.

### 1. `AWOPF.DBO.OUTLOAD` (Snowflake, ~3.3B rows) — per-bus load / participation factors

Columns: `RUNID`, `LABEL`, `BUSNUM`, `ID`, `MW`, `BUSNAME`, `AREANAME`, `ZONENAME`, `LOADSTATUS`, `BRANCHLOADID`, plus 5 `CUSTOMSTRING*`/5 `CUSTOMINTEGER*`/5 `CUSTOMFLOAT*` columns (mostly outage-metadata carried through from a 7.2026 schema change — `CUSTOMSTRING1`=outage reason, `CUSTOMINTEGER0/1`=outage begin/end, `CUSTOMFLOAT0`=LDF, `CUSTOMFLOAT4`=BranchLoadID; all NULL in the sampled run, so likely populated only for outage-linked load records).

Sample rows for RUNID=1946934 (in the study-14411 range 1946934-1947143) show individual bus loads like `IOLA_IOLA2 = 12.36 MW`, `ROANSPRARE = 7.40 MW`, `ZONENAME='WZ_EAST'`. **Row count for RUNID=1946934: 7,744 rows, `SUM(MW) = 76,742 MW`** — that total is a sane ERCOT system-load figure (well within the ~35k-85k MW historical range confirmed in `ercotLoadView` below), so bus-level `MW` values do sum to a sensible system load. **Key columns for participation-factor derivation: `BUSNUM` (bus key), `MW` (bus-level allocation for that RUNID/hour), `ZONENAME` (maps to `WZ_*` weather zones matching `ercotLoadView` naming) — a bus's participation factor = `MW / SUM(MW WHERE RUNID = same run)` (or scoped to its `ZONENAME` if allocating per-zone deltas rather than system-wide).**

### 2. Zonal load actuals

**SQL Server `AW.dbo.ercotLoadView`**: columns `date` (date), `he` (tinyint, hour-ending), `ERCOT_ALL_LD` (decimal, system total), and 7 zonal columns — `WZ_EAST_LD`, `WZ_NORTH_CENTRAL_LD`, `WZ_FAR_WEST_LD`, `WZ_WEST_LD`, `WZ_NORTH_LD`, `WZ_SOUTHERN_LD`, `WZ_COAST_LD`, `WZ_SOUTH_CENTRAL_LD` (all decimal). Sample rows confirm plausible hourly totals (28,647 MW to 85,435 MW across three random dates/hours) — this is the hourly zonal-load-actuals source, confirmed at grain (date, he).

**Snowflake equivalent search** (`SHOW TABLES LIKE '%LOAD%' IN AW.DBO`) found no direct `ercotLoadView` analog. Closest candidates: `SELOAD`/`SELOADSPP`/`SELOADDEVICE*` (state-estimator load snapshots, keyed by `secaseid` — a different grain/purpose, not simple hourly zonal actuals), `ERCOTSTATIONLOAD` (486M rows, station-level not zonal), and `LDFCSTLOAD` (70.4M rows — this turned out to be the **forecast** table, see section 7, not actuals). **No bulk hourly zonal-actuals equivalent was found in Snowflake AW.DBO** — `ercotLoadView` on SQL Server remains the source for zonal load actuals; treat as a gap if this needs to move off SQL Server for bulk history (small/periodic pulls of `ercotLoadView` are fine per this project's SQL Server rule of "targeted, capped queries only").

### 3. Solar actuals

**SQL Server `AW.dbo.solarGenAct`**: columns `awDateID` (int), `HE` (smallint), `isoMarketID` (smallint), `isoZoneID` (int), `priceTypeID` (smallint), `solarGen`/`solarMax`/`solarMin` (numeric). Sample rows: `isoMarketID=6` (ERCOT), `isoZoneID=67`, `priceTypeID=2`, `solarGen=0.9` (early-morning hour, near-zero as expected) with `solarMax`/`solarMin` NULL in the sample — those bound columns may only populate for certain `priceTypeID`/date ranges, worth checking a daytime hour before relying on them for CF bounds. `isoZoneID` needs a zone-dimension join (not resolved here) to map to WZ_* or a solar-specific region.

**Snowflake**: `SHOW TABLES LIKE '%SOLAR%' IN AW.DBO` returned zero rows — **no Snowflake equivalent exists**; `solarGenAct` on SQL Server is the only solar-actuals source found.

### 4. Wind region hourly (CF source)

**SQL Server `AW.dbo.ercotWindRegionHourly`**: columns `TimeStamp` (datetime), `windRegion` (varchar), `avgHSLPct` (float — average high-sustainable-limit percent, i.e. the capacity factor proxy), `totalInstalledCapacityMW` (float, region's installed capacity at that timestamp), `isActual` (bit), `lastUpdated` (datetime). Sample rows show regions `Coastal`, `North`, `Panhandle` with `avgHSLPct` 2.25/38.1/50.6 and matching `totalInstalledCapacityMW`. **Coverage: `MIN(TimeStamp) = 2014-12-03`, `MAX(TimeStamp) = 2026-08-10`** (current, continuously updated). This table conveniently carries both the CF (`avgHSLPct`) and the contemporaneous installed capacity per region in the same row, so wind CF draws don't need a separate capacity-through-time join for this specific breakdown — though it's regional (5-ish named wind regions), not per-generator.

### 5. Installed capacity through time

**SQL Server `AW.dbo.ercot_resource_adequacy_report_view`**: columns `REPORT_DATE` (date), `SOURCE` (varchar, e.g. `MORA_August2026` — ties to ERCOT's periodic MORA/CDR-style adequacy reports), `UNIT_NAME`, `UNIT_CODE`, `INR`, `COUNTY`, `COUNTY2`, `FUEL` (e.g. `STORAGE`, `WIND-O`), `ZONE`, `IN_SERVICE` (int — looks like an in-service **year**, values like 2022/2024 in the sample, not a boolean; confirm before treating as a flag), `INSTALLED_CAPACITY_RATING_MW` (float), `NOTES`. **Coverage: `MIN(report_date) = 2023-09-19`, `MAX(report_date) = 2026-06-08`** — periodic snapshots (one `SOURCE` label per report vintage), not continuous daily data; three years of vintages is enough to build capacity-rescaling factors for recent history but not a long back-series.

### 6. Fast-scan input side (Snowflake `AWDEV.FLOW_ANALYSIS`)

**`FAST_SCAN_RUN_LOG`**: `RUN_ID` (identity PK), `CONFIG_ID`, `START_DATE`, `END_DATE`, `ROWS_INSERTED`, `STATUS`, `ERROR_MESSAGE`, `RUN_AT`, `RUN_BY`. Sample rows all show `STATUS='EXISTING_CONFIG_ID'`, `ROWS_INSERTED=0`, `START_DATE`/`END_DATE` NULL — these look like config-registration/no-op log entries rather than completed data-generating runs; a `CONFIG_ID` dimension table (not explored — access wasn't granted to probe further tables in this database beyond the two named) presumably holds the actual run parameters (topology, date range, contingency set) that `CONFIG_ID` points to.

**`UNCONSTRAINED_DISPATCH_HOURLY`**: columns `GENID`, `DATE`, `HE` (composite PK with GENID+DATE), `TIMESTAMP`, `CPNODEID`, `RESOURCETYPE`, `UNCONSTRAINED_MW`, `UNCONSTRAINED_HB_MW`, `BASE_POINT_MW`, `DELTA_MW`, `TELEMETERED_MW`, `HSL`, `PRICE_USED`, `PRICE_SOURCE`, `INTERVAL_COUNT`, `COMPUTED_AT`. **This is exactly the per-unit dispatch/injection vector for fast-scan**: `UNCONSTRAINED_MW` per `GENID` per `(DATE, HE)` is the injection that would flow into `INJECTION_FLOW` in `FAST_SCAN_RESULTS` for that unit/hour, with `TELEMETERED_MW`/`BASE_POINT_MW`/`HSL` giving the actuals/base-point/headroom context used to derive it and `PRICE_USED`/`PRICE_SOURCE` suggesting an economic-dispatch or price-driven redispatch step feeds the "unconstrained" estimate. **However, `SELECT * ... LIMIT 3` returned zero rows** — the table exists with a fully-defined schema but is currently empty (or not populated for whatever date range/warehouse scope this query implicitly hit), so the apparent fast-scan injection pipeline is: `UNCONSTRAINED_DISPATCH_HOURLY` (per-unit MW by hour) → summed/mapped to bus injections → pushed through the SF matrix (`BUS_SHIFTS_VIEW`, section 8) → `FAST_SCAN_RESULTS.INJECTION_FLOW`. **Flag as a gap: table is empty right now — confirm with the fast-scan owner whether this is mid-build/paused, or whether injections are actually sourced elsewhere (e.g. directly from `OUTGEN`/OUTLOAD-style SCOPF output) before depending on it.**

### 7. ERCOT load forecast tables

`SHOW TABLES LIKE '%MTLF%'`, `'%LOADFORECAST%'`, and `'%FORECAST%'` all returned zero rows in `AW.DBO` — no table literally named with those tokens. The actual forecast table surfaced instead under the `%LOAD%` sweep in section 2: **`AW.DBO.LDFCSTLOAD`** (70.4M rows). `DESCRIBE TABLE`: `FORECASTID`, `ISOZONEID`, `RUNDATEID`, `RUNHOUR`, `FORECASTDATEID`, `FORECASTHE`, `FORECASTTYPEID`, `LOAD` (FLOAT). Sample rows: `ISOZONEID=53`, one forecast run (`RUNDATEID=44953`, `RUNHOUR=1`) producing multiple forward `(FORECASTDATEID, FORECASTHE)` rows with `LOAD` ~1,600-1,700 (units/zone scale consistent with a sub-system zone, not ERCOT-wide) and `FORECASTTYPEID=5` in all three samples. Grain: one row per (RUNDATEID, RUNHOUR, ISOZONEID, FORECASTDATEID, FORECASTHE, FORECASTTYPEID) — i.e. this is ERCOT's rolling MTLF/STLF-style forecast, published per run, per zone, for a range of forward hours. `RUNDATEID`/`FORECASTDATEID` are date-dimension surrogate keys needing the same date-dim join noted in section 7 of the original notes (`AWDATEID`-style). `FORECASTTYPEID` needs a dimension lookup to distinguish MTLF vs STLF vs other forecast types — not resolved here.

### 8. `BUS_SHIFTS_VIEW` practicalities

`SELECT COUNT(DISTINCT BUSNUM) ... WHERE FROMNUM=1436 AND TONUM=2081 AND CKT='1' AND CTGLABEL='DWCSRAM5' AND ABS(PSENS)>0.02` returned **11,719 distinct buses**. Note: as of the branch's known-blockers note (`powerflow_analytics/CLAUDE.md`, 2026-08-10), `SHIFT_FACTORS.DBO.BUS_SHIFTS_VIEW` was previously reported as not authorized to the `read_only` role — **this query succeeded**, so access has since been granted (or the blocker note is stale; flag for the coding agent to re-run `pfa/extract/shifts.py::probe_access()` and update `CLAUDE.md` accordingly). Sizing implication: with ~11.7k buses clearing the |PSENS|>0.02 threshold for a single constraint, a full per-constraint SF vector pull is a non-trivial but manageable single-constraint fetch (thousands of rows); pulling this for many constraints x many topologies at once would need batching/filtering (e.g. raise the PSENS threshold, or pre-aggregate to zone-level SF) to stay within reasonable single-query sizes.

### MC quickstart

Concrete data recipe for the linearized scenario Monte Carlo:

**(a) Historical joint (load, wind, solar) sample** — draw correlated historical hours from:
- Load: SQL Server `AW.dbo.ercotLoadView` (hourly, `ERCOT_ALL_LD` + 7 `WZ_*_LD` zonal columns, 2013+ coverage per samples seen).
- Wind: SQL Server `AW.dbo.ercotWindRegionHourly` (hourly `avgHSLPct` by `windRegion`, 2014-12-03 through today, continuously updated) — multiply CF by the row's own `totalInstalledCapacityMW` for that timestamp to reconstruct historical wind MW, or use `avgHSLPct` directly as the CF draw and rescale by current capacity (step b).
- Solar: SQL Server `AW.dbo.solarGenAct` (hourly `solarGen`/`solarMax`/`solarMin` by `isoZoneID`) — needs a zone-dimension join to resolve `isoZoneID`, and daytime-hour sampling to get non-trivial `solarMax`/`solarMin` bounds (not confirmed populated in the sampled overnight hour).
- Sample jointly by (date, HE) across the three sources to preserve correlation (a windy/sunny/low-load hour looks different from a calm/dark/peak-load hour) rather than drawing each margin independently.

**(b) Capacity/peak rescaling factors** — `AW.dbo.ercot_resource_adequacy_report_view` (SQL Server, periodic MORA/CDR-style snapshots, `REPORT_DATE` 2023-09-19 to 2026-06-08, per-unit `INSTALLED_CAPACITY_RATING_MW` by `FUEL`/`ZONE`) gives current vs. historical installed capacity by fuel/zone to rescale a historical wind/solar CF draw to today's fleet size before allocating MW. `ercotWindRegionHourly.totalInstalledCapacityMW` gives an even more direct per-region capacity-at-that-time figure for wind specifically, so wind rescaling can skip the adequacy-report join if region-level (not fuel/unit-level) granularity is sufficient.

**(c) Bus-level allocation** — `AWOPF.DBO.OUTLOAD` (Snowflake, per-`BUSNUM` `MW` for a `RUNID`, confirmed bus loads sum to a sane system total, `ZONENAME` matches the `WZ_*` convention used in `ercotLoadView`) gives load participation factors per bus/zone: `pf(bus) = MW(bus) / MW(zone or system)` from a representative run, then apply that factor to a scenario's sampled zonal/system load to get bus-level load injections. For wind/solar generator-level allocation, the gen-mapping side (`pfa/extract/gen_mapping.py`, not explored in this pass) presumably maps `GENID`/`BUSNUM` to fuel type/zone so a zonal wind or solar MW draw can be spread across the actual generator buses in that zone/region — cross-check that module before building the allocator.

**(d) SF matrix per constraint** — `SHIFT_FACTORS.DBO.BUS_SHIFTS_VIEW` (Snowflake), sliced by `(FROMNUM, TONUM, CKT, CTGLABEL)` for the constraint of interest, `BUSNUM`/`PSENS` gives the linear sensitivity of that constraint's flow to each bus's injection — confirmed accessible (11,719 buses returned for one sample constraint at |PSENS|>0.02) despite the branch's `CLAUDE.md` blocker note being apparently stale. Combine with (c)'s bus-level injections: `ΔFlow = Σ_bus PSENS(bus) × ΔMW(bus)`, then compare against the constraint's limit (from `AWOPF.DBO.OUTBRANCH2`/`OUTCTGVIOL2`, already used elsewhere in this project) to estimate P(bind); pair with a historical or fast-scan-derived lambda distribution for that constraint to estimate the lambda distribution conditional on binding.

**Gaps found**:
- No Snowflake equivalent for bulk hourly zonal load actuals (`ercotLoadView`) or solar actuals (`solarGenAct`) — both remain SQL-Server-only, so bulk historical sampling for the MC needs either periodic capped pulls or a request to mirror these into Snowflake.
- `AWDEV.FLOW_ANALYSIS.UNCONSTRAINED_DISPATCH_HOURLY` is empty despite a complete, on-point schema for per-unit dispatch injections — confirm with the fast-scan/data-eng owner whether it's mid-build or whether another source already feeds `FAST_SCAN_RESULTS.INJECTION_FLOW`.
- `FAST_SCAN_RUN_LOG` sampled rows are all no-op `EXISTING_CONFIG_ID` entries with null date ranges — the actual run-parameterization (topology/date range/contingency set) likely lives in a `CONFIG_ID`-keyed dimension table not yet identified/explored.
- `LDFCSTLOAD.FORECASTTYPEID` and `ercot_resource_adequacy_report_view.IN_SERVICE` semantics are not confirmed against a dimension table (the latter looks like an in-service year, not a boolean, based on sample values) — verify before using either in a model.
- `BUS_SHIFTS_VIEW` access appears to have changed since the branch's last documented blocker (2026-08-10 `CLAUDE.md` note said not authorized; this session's query succeeded) — re-verify and update that note so `pfa/extract/shifts.py` can stop degrading to the LPDELTAMW fallback if access is now solid.
